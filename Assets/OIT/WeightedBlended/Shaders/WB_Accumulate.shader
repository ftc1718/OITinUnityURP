...
Shader "Hidden/Weighted Blended/Accumulate" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			void frag(v2f i, float4 color : SV_Target0, float alpha : SV_Target1) {
				...
				#ifdef _WEIGHTED_ON
					color = float4(C, alpha) * w(i.z, alpha);
				#else
					color = float4(C, alpha);
				#endif
			}
